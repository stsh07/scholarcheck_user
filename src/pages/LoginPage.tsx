    ...
    switch (name) {
      case "email":
        if (!value.trim()) message = "Email is required.";
          ...
          message = `Maximum ${MAX_EMAIL_LEN} characters allowed.`;
        else if (!emailRegex.test(value))
          message = "Enter a valid email (lowercase, no spaces, ending with .com).";
        break;
      case "password":
        if (!value) message = "Password is required.";
        else if (!passwordRegex.test(value) || /\s/.test(value))
          message = "Incorrect password.";
        break;
    }

    setErrors((prev) => ({ ...prev, [name]: message }));
    return message; // return message so we can stop submit immediately
  };
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <div className="flex flex-col min-w-0 gap-1">
                ...
                <input
                  ...
                  value={email}
                  onChange={(e) => {
                    const val = e.target.value.replace(/\s/g, "").toLowerCase(); // remove spaces & lowercase
                    setEmail(val);
